...
namespace Spindles {
    ...
    void Dac::init() {
        ...
        // ESP32-S3 does not have built-in DAC
        _gpio_ok = false;
        grbl_msg_sendf(CLIENT_ALL, MsgLevel::Info, "DAC spindle not supported on ESP32-S3 (no built-in DAC)");
        return;
#else
    ...
    void Dac::set_output(uint32_t duty) {
        if (_gpio_ok) {
#ifdef CONFIG_IDF_TARGET_ESP32S3
            // ESP32-S3 has no built-in DAC - do nothing
#else
